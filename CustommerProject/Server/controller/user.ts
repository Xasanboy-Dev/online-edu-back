        ...
        if (phoneNumber && password) {
            ...
            if (user) {
                if (await ComparePassword(password, user.password)) {
                    ...
                    return res
                        .status(200)
                        .json({ message: 'All right', user: token })
                } else {
                    return res
                        .status(400)
...
export async function checkTokenValid(req: Request, res: Response) {
    try {
        const token = req.headers.authorization
        if (token) {
            const ValiToken: any = VerifyToken(token)
            let id = ValiToken.id
            const user = await getOneUserById(+id)
            if (user) {
                return res.status(200).json({ message: 'All right', user })
            } else {
    ...
    try {
        ...
        if (token) {
            ...
            if (user) {
                ...
                return res
                    .status(200)
                    .json({ message: 'All comments', comments })
            } else {
                return res
                    .status(404)
                    .json({ message: 'Your user is not exist!' })
