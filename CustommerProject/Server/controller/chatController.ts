  ...
  try {
    ...
    if (token && chatId) {
      ...
      if (chat) {
        ...
        return res.status(404).json({ message: "Your chat is not exist!" });
      }
    } else {
...
export async function addChat(req: Request, res: Response) {
  try {
    const token = req.headers.authorization;
    const { type, name }: { type: Type; name: string } = req.body;
    const { users }: { users: number[] } = req.body;
    if (token && type && users) {
      const ValidateToken = VerifyToken(token);
      let createdChat: chat;
      if (type == "group") {
        createdChat = await createChat("group", users, name ? name : "Chat");
      } else {
        createdChat = await createChat("chat", users, name ? name : "Chat");
      }
